...
# Usage: displayhelp
	...
	echo "     -k, --keep-dts   Keep external Surround Sound track (implies '-n')."
	echo "     -m, --nocolor    Do not use colors (monotone)."
	echo "     --md5            Perform MD5 comparison when copying across drives."
	echo "     -n, --no-dts     Do not retain the Surround Sound track."
	...
	echo "     -t TRACKID,"
	echo "     --track TRACKID  Specify alternate Surround Sound track."
	echo "     -w FOLDER,"
...
while [ -z "$MKVFILE" ]; do
	...
	case "$1" in
		...
		"-i" | "--initial" ) # Make new AC3 track the first in the file
			INITIAL=1
		;;
		"-k" | "--keep-dts" ) # Only allow external Surround Sound track if muxing AC3 track
			if [ -z $EXTERNAL ]; then
				KEEPDTS=1
			fi
		;;
		...
		* )
			...
			if [ $# -ne 0 ]; then
				...
				echo ""
				echo $"Control Flags:"
				printf "  %s: %s" $"Strip Surround Sound:" $NODTS
				printf "  %s: %s" $"Keep Surround Sound: " $KEEPDTS
				printf "  %s: %s" $"Set AC3 default: " $DEFAULT
				printf "  %s: %s" $"External AC3: " $EXTERNAL
				printf "  %s: %s" $"Surround Sound track: " $DTSTRACK
...
if [ "$(mkvmerge -i "$MKVFILE" | grep -i "${AUDIOTRACKPREFIX}AC-3")" ]; then
	...
	AC3CHANNELS=$(mediainfo --Inform='Audio;%Channels%' "$MKVFILE" | sed 's/[^0-9].*$//')
		if (($AC3CHANNELS > 2)); then
			color YELLOW; echo $"This mkv file already has a surround sound AC3 track. Removing subs if they exist then remuxing."; color OFF
			...
			color YELLOW; echo $"Found an AC3 audio file but it is stereo. Continuing..."
		fi
fi
...
if [ -z $DTSTRACK ]; then
	...
	if [ $EXECUTE = 1 ]; then
		...
		# Check to make sure there is a Surround Sound track in the MVK
		if [ -z $DTSTRACK ]; then
			color YELLOW; echo $"WTF...??  There are no Surround Sound tracks in '$MKVFILE'. If there are subtitles I'll remove them then I'm done!"; color OFF
	...
	if [ $EXECUTE = 1 ]; then
		...
		if [ -z "$VALID" ]; then
			error $"Track ID '$DTSTRACK' is not a Surround Sound track and/or does not exist."
			exit 1
		else
...
if [ -z $DTSNAME ]; then
	...
	doprint '> echo "$INFO" | grep -m 1 "Name" | cut -d " " -f 5- | sed "s/DTS|DTS....|EAC3.|E.AC3.|E.AC.3.|TRUE.HD./AC3/" | awk '"'{gsub(/[0-9]+(\.[0-9]+)?(M|K)bps/,"640Kbps")}1'"''
	DTSNAME="DTSNAME" #Value for debugging
	dopause
	if [ $EXECUTE = 1 ]; then
		DTSNAME=$(echo "$INFO" | grep -m 1 "Name" | cut -d " " -f 5- | sed -e "s/DTS*/AC3/; s/DTS.../AC3/; s/E.AC.3/AC3/; s/EAC3/AC3/; s/E.AC3/AC3/" | awk '{gsub(/[0-9]+(\.[0-9]+)?(M|K)bps/,"640Kbps")}1')
	fi
...
dopause
if [ $EXECUTE = 1 ]; then
	color YELLOW; echo $"Extracting Surround Sound Track: "; color OFF
	...
	checkerror $? $"Extracting Surround Sound track failed." 1
	timestamp $"Surround Sound track extraction took:	"
fi
...
dopause
if [ $EXECUTE = 1 ]; then
	color YELLOW; echo $"Converting Surround Sound to AC3:"; color OFF
	...
	checkerror $? $"Converting the Surround Sound file to AC3 failed" 1

	# If we are keeping the Surround Sound track external copy it back to original folder before deleting
	if [ ! -z $KEEPDTS ]; then
		color YELLOW; echo $"Moving Surround Sound track to MKV directory."; color OFF
		$RSYNCCMD "$DTSFILE" "$DEST"
		checkerror $? $"There was an error copying the Surround Sound track to the MKV directory. You can perform this manually from \"$DTSFILE\"." 1
	fi
	cleanup "$DTSFILE"
...
if [ $EXTERNAL ]; then
	...
	## If user doesn't want the original Surround Sound track drop it
	#if [ $NODTS ]; then
...
#			# If there is only the Surround Sound audio track then drop all audio tracks
#			CMD="$CMD -A"
#		else
...
if [ "$NEWFILEDEVICE" = "$DSTFILEDEVICE" ]; then
	...
	#SFE - CREATE NEW DIRECTORY FOR FINAL FILENAMES
	#mkdir "/mnt/DownloadHDD/SABNZBD/COMPLETE/movies/$PARENTNAME"
fi
...
if [ $EXECUTE = 1 -a $PAUSE = 0 ];then
	...
	printf "%23s %15d KB\n" $"Extracted Surround Sound Filesize:" $DTSFILESIZE|sed -e :a -e 's/\(.*[0-9]\)\([0-9]\{3\}\)/\1,\2/;ta'
